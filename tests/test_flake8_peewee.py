...
def test_trivial_case():
    assert _results("") == set()
    assert _results("UserPlan.select(UserPlan.user_id == 1).where()") == {"1:0 PWE101 select() inner comparison expression are not allowed"}
    ...
    print(up)""") == {"1:10 PWE101 select() inner comparison expression are not allowed"}
    ...
    try:
        ...
        current_app.logger.error(err, exc_info=True)""") == {"1:10 PWE101 select() inner comparison expression are not allowed"}
    assert _results("sc_models.UserCourse.select(sc_models.UserCourse.stage3_end_date >= date)") == {"1:0 PWE101 select() inner comparison expression are not allowed"}
